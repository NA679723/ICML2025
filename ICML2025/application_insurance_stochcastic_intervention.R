...
for(boot in 1:BOOT){
  ...
  for(boo in 1:BOO){
    ...
    CACE1<-CACE1+(alpha1-alpha0)*(max(Y)-min(Y))/(BOO*length(X))
    PCACE1<-PCACE1+max(alpha1-alpha0,0)*(max(Y)-min(Y))/(BOO*length(X))
    NCACE1<-NCACE1+min(alpha1-alpha0,0)*(max(Y)-min(Y))/(BOO*length(X))
